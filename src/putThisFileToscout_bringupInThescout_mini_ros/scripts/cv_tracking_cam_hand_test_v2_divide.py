...
from hand import hand_detection
...
# ========= hand option에 따라 움직임을 제어할때 사용 ===================
# move 시 linear.x 가 0.2로 세팅되도록 되었는데 publish차이가 있는지?
...
def main(video_path,pub):
    ...
    # 30 frame 반복당 tracker initialize
    DETECT_AFTER = 30
    ...
    while cap.isOpened():
        ...
        #비디오 화면에 읽혀지지 않으면 break
        if not ret:
            break
        
        #cam의 화면과 손의 위치가 동일하도록 화면을 좌우반전
        ...
        #기존에 학습된 YoloV4_tiny 모델로 frame_input을 추론하여, '사람' 클래스의 바운딩 박스를 결과로 받아온다.
        #infer = saved_model_loaded.signatures['serving_default']
        #'checkpoints/yolov4-tiny-416'
        ...
        # init_first : 첫 main 코드를 돌 때 True로 설정되어 있어, 프로그램의 첫 동작을 의미
        # num_objects : num_objects = valid_detections.numpy()[0] 한 값으로, 현재 프레임 안에 사람이 1명인지 여부를 나타냄
        if init_first :
            if num_objects ==1:
            # cap 이 진행중임
            # action= ''
            # move or stop  동작을 받아야 다음 step 가능
                ...
                #오른쪽에 위치한 손의 origin 좌표를 받아올 수 있도록
                #손 위치를 detection하여 받아온다.
                    ...
                    # =============== hand detection end =================
                    # tracker 초기화
                    # get_coordinates : person 클래스 중에 핸드와 일치하는 하나의 박스 그리기 
                    ...
                    #opencv tracker 알고리즘에 따라 tracking
                    tracker = OPENCV_OBJECT_TRACKERS[algo]()
                    ...
                    #tracker가 success 하였으면 init_first는 false라고 봄
                    init_first = False

        # !init_first:
        else:
            #tracker 추적이 계속 유지되는 경우 다음과 같이 박스를 그려 표시함
            if success :
            ...
            # hand detection 을 통해 '추적대상' 을 정했고    
            # 2번째 이상의 프레임에서, 추적을 잃었거나 갱신 단위에 해당하는 경우
            # or not success 부분을 생략함(위의 if에서 걸러졌을 것으로 판단) 
                
                # num_objects가 1보다 작은 경우(놓친 경우)
                # num_objects <1 == !success
                if num_objects < 1 :
                    ...
                    #num_objects 가 1개 이상일 때
                    if frame_number % DETECT_AFTER == (DETECT_AFTER-1) :
