...
def image_preprocess(image, target_size, gt_boxes=None):
    ...
    # bounding box 형식 변경 ymin, xmin, ymax, xmax ---> xmin, ymin, width, height
...
def draw_bbox(image, bboxes, info = False, show_label=True, classes=read_class_names(cfg.YOLO.CLASSES)):
    ...
    #print(f"len out_boxes[1]  :  {len(out_boxes)}  ")
    #print(f"out_boxes  :  {out_boxes}  ")
    #print(f"out_boxes[1]  :  {out_boxes[0]}  ")
    
    #print(f"num_boxes  :  {num_boxes}  ")
    for i in range(num_boxes):
        if int(out_classes[i]) < 0 or int(out_classes[i]) > num_classes :#or len(out_boxes[i]) != num_boxes : 
            continue
        x,y,w,h = out_boxes[i]
        fontScale = 0.5
        score = out_scores[i]
        ...
        if show_label:
            ...
            cv2.putText(image, bbox_mess, (c1[0], np.float32(c1[1] - 2)), cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale, (250, 250, 250), (bbox_thick // 2) +1, lineType=cv2.LINE_AA)
    return image
