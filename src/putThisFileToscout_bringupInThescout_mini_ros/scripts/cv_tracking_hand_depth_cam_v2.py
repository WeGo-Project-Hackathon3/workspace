...
def get_move(W,track_w, H=0, track_H=0):
    ...
    #angular_z += (z_bias_2*angular_chk)
    # 최소값 정의 angular
    #if abs(angular_z) <= 0.37+ abs(z_bias) :
    if abs(angular_z) <= 0.1+ abs(z_bias_1) :
        angular_z = 0.0
    ...
    #while cap.isOpened() :
        ...
        if not init_first and frame_number % DETECT_AFTER == (DETECT_AFTER-1) or not success  :
            if num_objects>=1 :
                ...
                #initBB = hand_initBB 
                print("pre bboxes : ", bboxes, "   type : ", type(bboxes))

                new_bboxes = get_box_distance(bboxes, [x, y, x+w, y+h],  W)
                print("post bboxes : ", new_bboxes, "   type : ", type(new_bboxes))
                initBB, trueBB,iou_ = get_coordinates(new_bboxes, x, y, x+w, y+h)
        ...
        if abs(angular_z_pre) - abs(angular_z) > 0.25 + z_bias: 
            ...
            time_wait_pub(1, pub,linear=0.35, angular=angular_z_pre)
